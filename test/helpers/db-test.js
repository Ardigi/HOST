...
async function createTables(client) {
	...
	await client.execute(`
		...
		VALUES ('user-1', 'test@example.com', 'server', ${Date.now()}, ${Date.now()})
	`);
	// Create orders table
	await client.execute(`
 ...
 */
export async function cleanupTestDatabase(_db) {
	if (testClient) {
		testClient.close();
		testClient = null;
		testDb = null;
 ...
 */
export async function resetTestDb(client) {
	// Get all table names
 ...
 */
export async function seedTestData(_client) {
